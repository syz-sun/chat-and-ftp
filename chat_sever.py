...
def do_login(s,name,address):
    if name in user or "管理" in name:
        ...
        s.sendto("您的IP已被拉黑,不能进入聊天室".encode(),address)
    else:
...
def do_chat(s,name,text):
    list_sensitive_word=["xx","aa","bb","oo"]
    for i in list_sensitive_word:
        if i in text:
            text = "***"
    if text=="***":
        if name in record:
            ...
            record[name] = 1
        if record[name]<3:
            msg="%s发送了敏感信息,黄牌警告,请大家文明用语,%s的不良言论已被系统记录%d次，累计三次将被永久踢出聊天室."%(name,name,record[name])
            for i in user:
                s.sendto(msg.encode(),user[i])
        else:
            msg="%s发送敏感信息已达三次,已被踢出聊天室,请大家文明用语."%name
            for i in user:
                s.sendto(msg.encode(),user[i])
